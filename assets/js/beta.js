...
function createCert(domain, alternateNames, wildcard, days) {
...
$(() => {
	$("html").on("click", ".button", (e) => {
		...
		switch (action) {
			case "createCert":
				...
				let names = form.find("input[name=names]").val();
				...
				if (names.length) {
					...
					if (split.length) {
						alternateNames = [...new Set(split)];
					}
				}

				createCert(domain, alternateNames, wildcard, days).then(() => {
	...
	$("html").on("change", "input[type=checkbox]", (e) => {
		...
		switch (name) {
			case "alternateNames":
				if (checked) {
					$("input[name=names]").closest(".field").removeClass("hidden");
				}
				else {
					$("input[name=names]").closest(".field").addClass("hidden");
				}
				break;
		}
	});
